...
WATCHDOG_SCRIPT = PROJECT_ROOT / "auto_trader_watchdog.py"
SITE_SIGNAL_GENERATOR_SCRIPT = PROJECT_ROOT / "site_signal_generator_runner.py"
GUARDIAN_SCRIPT = PROJECT_ROOT / "mt5_trade_guardian.py"
...
def _process_matches_account(joined: str, account_id: str) -> bool:
    target = str(account_id or "").strip().lower()
    if not target:
        return False
    normalized = str(joined or "").lower().replace("\\", "/")
    return f"--account-id {target}" in normalized or f"accounts/{target}/" in normalized


def find_account_service_pids(account_id: str, script_name: str) -> list[int]:
    """ابحث عن عمليات خدمة تابعة لمحفظة محددة بدون لمس محافظ أخرى."""
    if psutil is None:
        return []

    target_script = str(script_name or "").strip().lower()
    ...
    for proc in psutil.process_iter(["pid", "cmdline"]):
        try:
            cmdline = proc.info.get("cmdline") or []
            joined = " ".join(str(x) for x in cmdline).lower()
            if target_script in joined and _process_matches_account(joined, account_id):
...
def find_account_service_pid(account_id: str, script_name: str) -> int:
    pids = find_account_service_pids(account_id, script_name)
...
def stop_account_runtime_services(account_id: str) -> list[int]:
    """أوقف خدمات محفظة واحدة فقط: المولد، المشرف، المتداول، والحارس."""
    pids = []
    for script_name in (
        ...
        pids.extend(find_account_service_pids(account_id, script_name))
...
def stop_conflicting_traders(target_account_id: str) -> list[int]:
    ...
    - يوقف أي continuous_auto_trader قديم بدون --account-id.
    - يترك عمليات الحسابات الأخرى المستقلة تعمل.
...
def build_watchdog_launch_command(config_path: Path, state_path: Path, wallet_path: Path, account_id: str) -> list[str]:
    return [
        sys.executable,
        "-u",
        str(WATCHDOG_SCRIPT),
...
def build_site_generator_launch_command(config_path: Path, interval_seconds: int = 45) -> list[str]:
    return [
        sys.executable,
        "-u",
        str(SITE_SIGNAL_GENERATOR_SCRIPT),
        "--config", str(config_path),
        "--interval-seconds", str(max(15, int(interval_seconds or 45))),
    ]


def build_guardian_launch_command(config_path: Path, wallet_path: Path, account_id: str) -> list[str]:
    return [
        sys.executable,
        "-u",
        str(GUARDIAN_SCRIPT),
...
def start_account_service(account_id: str, label: str, script_name: str, command: list[str], out_path: Path, err_path: Path, env: dict | None = None) -> dict:
    existing_pid = find_account_service_pid(account_id, script_name)
    if existing_pid > 0:
        return {"label": label, "status": "existing", "pid": existing_pid}
    ...
    run_env["AUTO_TRADER_ACCOUNT_ID"] = str(account_id or "").strip()

    creationflags = 0
    if os.name == "nt":
        creationflags = getattr(subprocess, "CREATE_NEW_PROCESS_GROUP", 0)

    with open(out_path, "a", encoding="utf-8", buffering=1) as out_file, open(err_path, "a", encoding="utf-8", buffering=1) as err_file:
        out_file.write(f"\n===== [{account_id}] {label} start {time.strftime('%Y-%m-%d %H:%M:%S')} =====\n")
    ...
    return {"label": label, "status": "started", "pid": int(process.pid)}


def strategy_needs_guardian(config_data: dict) -> bool:
    if bool(config_data.get("dry_run", False)):
        return False
    return bool(
        config_data.get("trailing_stop_enabled", False)
        or config_data.get("no_loss_after_profit_enabled", False)
        or is_site_signal_strategy(config_data)
    )
...
class AccountCard:
    def __init__(self, parent, account_row: dict, on_status_change=None):
        ...
        self.risk_pct_var = tk.StringVar(value=str(self.config_data.get("risk_percent", self.config_data.get("max_risk_percent_of_equity", 1.0))))
        ...
        row += 1
        self._add_entry(row, "💰 أقصى خسارة يومية USD (0=نسبة الريسك)", self.daily_loss_var)
        row += 1
    ...
    def _on_strategy_preset_change(self, event=None):
        ...
        current_risk_percent = self.risk_pct_var.get().strip()
        ...
        if keep_user_symbols:
            ...
            try:
                risk_percent = float(current_risk_percent or "1.0")
                self.config_data["risk_percent"] = risk_percent
                self.config_data["max_risk_percent_of_equity"] = risk_percent
            except ValueError:
                pass
        ...
        if "max_risk_percent_of_equity" in preset and not keep_user_symbols:
    ...
    def collect_account_payload(self):
        ...
        daily_loss_limit_usd = float(self.daily_loss_var.get().strip() or "200")
        config_data["daily_loss_limit_usd"] = daily_loss_limit_usd
        if daily_loss_limit_usd <= 0:
            config_data["daily_loss_limit_percent_of_equity"] = risk_percent
    ...
    def start_trading(self):
        ...
        try:
            ...
            site_signal_mode = is_site_signal_strategy(self.config_data)
            ...
            service_results = []
            if site_signal_mode:
                generator_interval = int(self.config_data.get("site_signal_generator_interval_sec", 45) or 45)
                service_results.append(
                    start_account_service(
                        account_id=self.account_id,
                        label="مولد إشارات الموقع",
                        script_name="site_signal_generator_runner.py",
                        command=build_site_generator_launch_command(self.config_path, generator_interval),
                        out_path=self.state_path.parent / "site_signal_generator.out.log",
                        err_path=self.state_path.parent / "site_signal_generator.err.log",
                        env=env,
                    )
                )

            existing_trader_pid = find_account_trader_pid(self.account_id)
            existing_watchdog_pid = find_account_service_pid(self.account_id, "auto_trader_watchdog.py")
            if existing_trader_pid > 0 or existing_watchdog_pid > 0:
                service_results.append(
                    {
                        "label": "مشرف المتداول",
                        "status": "existing",
                        "pid": existing_watchdog_pid or existing_trader_pid,
                    }
                )
            else:
                service_results.append(
                    start_account_service(
                        account_id=self.account_id,
                        label="مشرف المتداول",
                        script_name="auto_trader_watchdog.py",
                        command=build_watchdog_launch_command(self.config_path, self.state_path, self.wallet_path, self.account_id),
                        out_path=self.state_path.parent / "watchdog.out.log",
                        err_path=self.state_path.parent / "watchdog.err.log",
                        env=env,
                    )
                )

            if strategy_needs_guardian(self.config_data):
                service_results.append(
                    start_account_service(
                        account_id=self.account_id,
                        label="حارس الصفقات",
                        script_name="mt5_trade_guardian.py",
                        command=build_guardian_launch_command(self.config_path, self.wallet_path, self.account_id),
                        out_path=self.state_path.parent / "guardian.out.log",
                        err_path=self.state_path.parent / "guardian.err.log",
                        env=env,
                    )
                )

            # تحديث الحالة
            main_pid = find_account_trader_pid(self.account_id) or find_account_service_pid(self.account_id, "auto_trader_watchdog.py")
            self._update_status(f"🟢 يعمل (PID: {main_pid or 'بدء'})", "green")
            ...
            for item in service_results:
                status_text = "يعمل" if item.get("status") == "existing" else "بدأ"
                service_lines.append(f"- {item.get('label')}: {status_text} (PID: {item.get('pid')})")
            messagebox.showinfo(
                "✅ بدء التداول",
                f"بدأت خدمات الحساب {self.account_id}\n" + "\n".join(service_lines) + extra,
            )
        except Exception as exc:
    ...
    def stop_trading(self):
        ...
        stopped_service_pids = stop_account_runtime_services(self.account_id)
        if stopped_service_pids:
            self.process = None
            ...
            messagebox.showinfo("✅ تم الإيقاف", f"تم إيقاف خدمات الحساب {self.account_id}\nPIDs: {', '.join(str(x) for x in stopped_service_pids)}")
            return
